...
def test_snapshot():
  ...
  vfs.write_block('A', 2, bytearray(b'is'))
  vfs.write_block('A', 4, bytearray(b'2k'))
  vfs.write_block('A', 5, bytearray(b'mmr'))
  ...
  def print_disk():
    for i in range(1,6):
      ...
      if sz > 0: print(block_info[:sz].decode('utf-8'))
    print('----------------')
  print_disk()
  check_id = vfs.create_checkpoint('A')
  print('snapshot',check_id)
  vfs.write_block('A', 1, bytearray(b'sahil'))
  vfs.write_block('A', 2, bytearray(b'is'))
  vfs.write_block('A', 3, bytearray(b'noob'))
  vfs.write_block('A', 4, bytearray(b'1k'))
  vfs.write_block('A', 5, bytearray(b'trash'))
  print_disk()
  vfs.rollback('A',check_id)
  assert vfs.block_metadata[2].free  
  print_disk()
